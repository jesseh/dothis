...
from helpers import (setup_session, teardown_session, create_volunteer, create_duty,
...
def when_he_creates_a_duty_called_group1(step, duty_name):
    create_duty(duty_name)


@step(u'Then he sees that the duty "([^"]*)" was created')
def then_he_sees_that_the_duty_group1_was_created(step, duty_name):
    assert_in(duty_name, body())
